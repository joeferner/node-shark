...
var TcpTracker = module.exports = function() {
  this.sessions = {};
...
TcpTracker.prototype.track = function(packet) {
  ...
  if (!session) {
    session = {
      ...
      packetCount: 0,
      startTime: new Date(),
      isPacketFromSource: function(packet) {
        return !this.isPacketFromDestination(packet);
      },
    ...
    this.emit('start', session, packet);
  }

  session.packetCount++;
  session.lastPacketTime = new Date();
