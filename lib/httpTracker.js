...
var HttpTracker = module.exports = function(tcpTracker) {
...
HttpTracker.prototype._populateRequestOrResponse = function(requestResponse, packet) {
  var self = this;
  Object.keys(packet.http).forEach(function(key) {
    if (nodeshark.isStandardKey(key)) {
      return;
    }
    if (key == '\\r\\n') {
      return;
    ...
    requestResponse.headers[self._headerKeyToDisplay(key)] = packet.http[key].value;
  });
};
